...
def parse_date_from_entry(m2_tile_name: str) -> datetime | None:
    """Extracts the date from a MERRA-2 Tile Name.

    Args:
        m2_tile_name (str): Name of MERRA-2 tile.

    Returns:
        Parsed date or None.
    """
    match = re.search(r"\.(\d{7})T", m2_tile_name)
    if match:
...
def find_closest_tile(
    ...
    """Find Closes MERRA-2 Tile.

    MERRA-2 dataset gets updated every 2 or 3 days and each tile is marked by the time of
    ...
    Args:
        tile_queries (dict[str, tuple[str, list[str]]]): A dict with tile_query as key
            ...
            retrieved as value.
        tile_database (dict[str, list[str]]): A database mapping MERRA-2 tile_id to a list of
    ...
    query_results = pd.DataFrame(
        {"tile_queries": query_results.keys(), "m2_tiles": query_results.values()}
    )
    return query_results


def retrieve_m2_metadata(tile_info_df: pd.DataFrame) -> dict[str, list[str]]:
    """Retrieve MERRA-2 Tiles Metadata.

    Given a tile_id, start_date and end_date, this function fetches all the MERRA-2 granules
    ...
    Returns:
        A dictionary mapping tile_id to a list of available MERRA-2 granules.
    """
    granules_dict = {}
    for _, (
        tile_id,
...
def get_m2_tiles(data: pd.DataFrame, min_count: int = 100) -> pd.DataFrame:
    """Get MERRA-2 Tile ID for Each Observation.
    ...
    We first add the MERRA-2 tile ID for each observationa and count the number of
    ...
    Args:
        ...
        min_count: minimum count of locust observations per MERRA-2 tile.
...
def add_m2_granules(
    ...
    """Add MERRA-2 Granules.

    Data contains tile_id and a series of date for which the tile is desired. This
    function takes the tile_id and the dates and finds the MERRA-2 tiles closest to the
    ...
    Args:
        ...
        temporal_tolerance (int): Tolerance (in days) for finding closest MERRA-2 tile.

    Returns:
        A dataframe containing a list of MERRA-2 granules. Each granule is a directory
        containing all the bands.
    """
    tiles_info, tile_queries = get_m2_tile_info(
    ...
    tile_database = retrieve_m2_metadata(tiles_info)
